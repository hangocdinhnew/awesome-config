...
local interface = "eno1" -- Set the interface name here
    ...
    callback  = function()
        awful.spawn.easy_async_with_shell(string.format('bash -c "%s"', command),
            function(stdout, stderr, reason, exit_code)
                ...
                if(ip ~= "") then
                    ip_widget.widget.text = string.format(" Leaked IP: %s ", ip)
                else
                    ip_widget.widget.text = string.format(" Leaked IP: %s is down ", interface)
                end
        end)
    end
}
